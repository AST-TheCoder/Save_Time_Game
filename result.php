...
<body>
  	<?php $gr_sz =$_SESSION['gr_sz'];?>
	<?php
		$cnt=0;
		$sz=$gr_sz*$gr_sz;
		while($cnt<$sz){
			$val[$cnt]=$_SESSION['vals'][$cnt];
		...
		<?php for($i=1; $i<=$gr_sz; $i=$i+1){?>
			<tr>
				<?php for($j=1; $j<=$gr_sz; $j=$j+1){?>
					<td>
						<?php $val=$_SESSION['vals'][($i-1)*$gr_sz+$j-1];?>
	...
	<form action="start.php" method="POST">
		<?php if($sum==$dis[$sz-1]){?>
	    	<label for="resume">Great <?php echo $_SESSION['name'];?>, Your prediction is right.</label><br>
	    <?php }?>
		<?php if($sum!=$dis[$sz-1]){?>
	    	<label for="resume">Sorry <?php echo $_SESSION['name'];?>, Your prediction is Wrong.</label><br>
